...
do
    ...
    cout << "Okay " + name + ", first things first.  Do you want to see what " << endl
...
void Deal( int deck[], int play[][3] )
{
	...
	/* deal cards by passing addresses of card values from
...
void PickUp( int deck[], int play[][3], int column )
{
	int card = 0, row = 0;
        ...
        for (row = 0; row < 7; row++) {
            deck[card++] = play[row][top];
        }
        for (row = 0; row < 7; row++) {
            deck[card++] = play[row][column];
        }
        for (row = 0; row < 7; row++) {
            deck[card++] = play[row][bottom];
        }
